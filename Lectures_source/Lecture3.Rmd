...
- Specifically, we want to know:
  ...
  + Considering male and female data separately, do mean LH levels depend on light regime?
  
  + Do mean LH levels depend on LRF dosage?
  
  + Is there an interaction between LRF dosage and light regime?
  
  + Do any of these effects differ significantly between males and females?
...
- Considering male rat data first.
...
- Now consider female rat data.

```{r, echo = TRUE}
dat_wide <- read_csv("dat/datasets/Chapter 12/LHFemale.csv")
dat_wide
```
